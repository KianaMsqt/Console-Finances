...
for ( let i = 0; i < totalMonths - 1; i++ ) {
	...
	totalChanges += parseInt( monthlyChanges[ i ] );
	
	// The greatest increase in profits (date and amount) over the entire period.
	// The greatest decrease in losses (date and amount) over the entire period.
	greatestIncrease = Math.max( ...monthlyChanges );
	greatestDecrease = Math.min( ...monthlyChanges );
	
	if ( monthlyChanges[ i ] === greatestIncrease ) {
		greatestIncreaseDate = finances[ i + 1 ][ 0 ];
	} else if ( monthlyChanges[ i ] === greatestDecrease ) {
